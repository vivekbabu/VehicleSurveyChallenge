...
public class AbstractTimeAverageSpeedReportTest {
	@Rule
	...
	public void testGenerateReport() {
		...
		when(allVehiclesData.getAverageSpeed('A', 3600000))
				.thenReturn(averageSpeedForLaneA);
		AbstractTimeAverageSpeedReport timeAverageSpeedReport = getAbstractTimeAverageSpeedReport();
		timeAverageSpeedReport.generateAndDisplayReport(allVehiclesData);
		String reportPrinted = log.getLog();
		Assert.assertTrue(reportPrinted.contains("| 01:00  |30.0|"));
		Assert.assertEquals(23, StringUtils.countMatches(reportPrinted, "|0.0|"));
		
	}
	
	private AbstractTimeAverageSpeedReport getAbstractTimeAverageSpeedReport() {
		return new AbstractTimeAverageSpeedReport(){
